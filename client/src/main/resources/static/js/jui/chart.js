...
jui.define("chart.axis", [ "util.base" ], function(_) {
  ...
  var Axis = function(chart, originAxis, cloneAxis) {
    ...
    this.reload = function(options) {
      ...
      //[수정]
      var areadHeight = area.height;
      if(chart.get("brush")[0].type.indexOf("line") > -1){
        if((area.height-(_.getGlobalLegend()*20)) > 0){
          if((area.height /2) <= (area.height-(_.getGlobalLegend()*20))){
            areadHeight = area.height-(_.getGlobalLegend()*20);
          }else{
            areadHeight = area.height;
          }
        }
      }
      
      _area = calculatePanel(_.extend(options.area, {
        x: 0, y: 0 , width: area.width, height: areadHeight /*Legend의 줄바꿈이 일어난 만큼 Axis의 영역을 적게 표현 합니다.*/
      }, true), _padding);
...
jui.defineUI("chart.builder", [ "util.base", "util.dom", "util.svg", "util.color", "chart.axis" ], function(_, $, SVGUtil, ColorUtil, Axis) {
  ...
  var UI = function() {
    ...
    function fakeDrawWidget(self) {
      var draws = _widget;
      
      if(draws != null) {
        for(var i = 0; i < draws.length; i++) {
          if(draws[i].type == "legend"){
            ...
            // 위젯 기본 프로퍼티 정의
            var draw = new Obj(self, _axis[0], draws[i]).draw();
            draw.remove();
          }
        }
      }
    ...
    function initCanvasElement(self) {
      ...
      for(var key in _canvas) {
        ...
        elem.setAttribute("width", size.width+"px");
        elem.setAttribute("height", size.height+"px");
    ...
    function resetCanvasElement(self, type) {
      ...
      var size = getCanvasRealSize(self);
      //Canvas Resize 하도록 수정
      if(type == "main"){
        var c = document.getElementById(self.selector.substring(1)).getElementsByTagName("canvas");
        for(_idx in c){
          if(_.typeCheck("object",c[_idx])){
            c[_idx].setAttribute("width", size.width+"px");
          }
        }
      }
...
jui.define("chart.theme.monitoring", [], function() {
...
jui.define("chart.theme.dark", [], function() {
  ...
  return {
    ...
    backgroundColor : "#2e3442", /*관리 > 대시보드 Backgroud color 로 변경*/
    colors : themeColors,
...
jui.define("chart.theme.monitoringdark", [], function() {
  var themeColors = [
    "#12f2e8",
    "#26f67c",
    "#e9f819",
  ...
  return {
    fontFamily : "arial,Tahoma,verdana",
    /*backgroundColor : "#222222",*/
    backgroundColor : "#393e4d", /*관리 > 대시보드 Backgroud color 로 변경*/
    colors : themeColors,
    ...
    tooltipPointFontSize : 11,
    /*tooltipPointFontColor : "#868686",*/
    tooltipPointFontColor : "#eaeaea", /*display:'max' 했을 때 보여지는 Fon*/
    barFontSize : 11,
    barFontColor : "#868686",
    barBorderColor : "none",
    barBorderWidth : 0,
...
/*jui.define("chart.polygon.core", [ "chart.vector", "util.transform", "util.math", "util.base" ], function(Vector, Transform, math, _) {
...
/*jui.define("chart.grid.draw3d", [ "util.base", "chart.polygon.grid", "chart.polygon.line", "chart.polygon.point" ], function(_, GridPolygon, LinePolygon, PointPolygon) {
...
jui.define("chart.grid.core", [ "util.base", "util.math", "chart.grid.draw2d"], function(_, math, Draw2D, Draw3D) {
    ...
    this.drawBefore = function() {
      //if(!this.axis.cacheXY) {
        var sortFunc = jui.include("chart.topology.sort." + this.grid.sort),
          sortArgs = [ this.axis.data, this.axis.area(), this.grid.space, this.grid.machineExistLevel];
      ...
      this.scale = (function() {
        return function(index) {
          ...
          if(_.typeCheck("integer", index)) {
            ...
            var y = self.axis.cacheXY[index].y + self.axis.cache.viewY;
            var scale = self.axis.cache.scale;
...
jui.define("chart.brush.core", [ "util.base", "util.dom" ], function(_, $) {
  ...
  var CoreBrush = function() {
     ...
     * @return {Array}
     */
    this.getXYCustom = function(isCheckMinMax){
      ...
      func(function(i, group) {
        ...
        for(var j = 0; j < targetLength ; j++) {
          ...
          if(isCheckMinMax !== false) {
            isMin[key] = (value>0)?((!isMin[key])?value == series[key].min:false):false;
            if(isMin[key]){
              if(minCount[key] == undefined || minCount[key] == null){
                minCount[key] = 1;
              }else{
                minCount[key]+= 1;
              }
            }
            isMax[key] = (value>0)?((!isMax[key])?value == series[key].max:false):false;
            if(isMax[key]){
              if(maxCount[key] == undefined || maxCount[key] == null){
                maxCount[key] = 1;
              }else{
                maxCount[key]+= 1;
              }
            ...
            if(minCount[key] == 1){
              xy[j].min[i] = isMin[key];
            }else{
              xy[j].min[i] = false;
            }
            
            if(maxCount[key] == 1){
              xy[j].max[i] = isMax[key];
            }else{
              xy[j].max[i] = false;
            }
          }
 ...
 *!/
 ...
 outerPadding: 15,
 /!** @cfg {Boolean} [showText=false] Configures settings to var the percent text of a full stack bar revealed. *!/
 showText: false
 };
 };
      ...
      this.eachData(function(data, i) {
        ...
        for (var j = 0; j < brush.target.length; j++) {
          ...
          var startY = axis.y(data[brush.target[j]]),
            ...
            eY = zeroY,
            eIndex = 0;
          //activeEvents 추가에 따른 수정
          if(this.brush.activeEvents.length == 0) {
            //높이에 맞게 Rendering 되도록 변경
            if (startY <= zeroY) {
              while (eY > startY) {
                ...
                var r = chart.svg.rect({
                  ...
                  y: eY - unitHeight,
                  width: barWidth,
                  height: eY / Math.abs(eY / unitHeight)/*unitHeight*/,
                  fill: this.color(Math.floor(eIndex / (eY / brush.gap)))/*this.color(Math.floor(eIndex / brush.gap))*/,
                  "fill-opacity": 1
                });
          ...
          }else if(this.brush.activeEvents.length > 0) {
            var isHighLight = false;
            for (var _idx in this.brush.activeEvents) {
              if (axis.x.domain()[i] == this.brush.activeEvents[_idx]) {
                isHighLight = (axis.x.domain()[i] == this.brush.activeEvents[_idx]);
              }
            }
            //높이에 맞게 Rendering 되도록 변경
            if (startY <= zeroY) {
              while (eY > startY) {
                ...
                var r = chart.svg.rect({
                  ...
                  height: eY / Math.abs(eY / unitHeight)/*unitHeight*/,
                  fill: this.color(Math.floor(eIndex / (eY / brush.gap)))/*this.color(Math.floor(eIndex / brush.gap))*/,
                  "fill-opacity": isHighLight ? 1 : 0.2 /*activeEvents에 따라 Hi-Light 들어가게 수정*/
                });
              ...
              while (eY < startY) {
                ...
                var r = chart.svg.rect({
                  ...
                  height: unitHeight,
                  fill: this.color(Math.floor(eIndex / brush.gap)),
                  "fill-opacity": isHighLight ? 1 : 0.2
                });
    ...
    this.createTooltip = function(g, pos, index) {
      ...
      for (var i = 0; i < pos.x.length; i++) {
        if((display == "max" && pos.max[i]) || (display == "min" && pos.min[i]) || display == "all") {
          ...
          if(keepMaxValue != pos.value[i]){
            keepMaxValue =　pos.value[i];
...
jui.define("chart.brush.scatter", [ "util.base" ], function(_) {
   ...
   * @extends chart.brush.core
   */
  var ScatterBrush = function(util, axis) {
      ...
      for(var i = 0; i < points.length; i++) {
        for(var j = 0; j < points[i].length; j++) {
          ...
          if((d == "max" && data.max) || (d == "min" && data.min) || d == "all") {
            if(this.brush.activeEvents != undefined && this.brush.activeEvents != null && this.brush.activeEvents.length > 0){
              var isHighLight = false;
              for (var _idx in this.brush.activeEvents) {
                if (axis.x.domain()[j] == this.brush.activeEvents[_idx]) {
                  isHighLight = (axis.x.domain()[j] == this.brush.activeEvents[_idx]);
                }
              }
              g.append(this.drawTooltip(data.x, data.y, this.format(data.value), isHighLight));
            }else{
              g.append(this.drawTooltip(data.x, data.y, this.format(data.value), true));
            }
          }
    ...
    this.drawTooltip = function(x, y, text, isHighLight) {
      if(x == undefined || x == null || isNaN(x)){
        ...
        var textTooltip = this.chart.text({
          y: -this.brush.size,
          "text-anchor" : "middle",
          fill : this.chart.theme("tooltipPointFontColor"),
          "font-size" : this.chart.theme("tooltipPointFontSize"),
...
jui.define("chart.brush.topologynode", [ "util.base", "util.math", "chart.topology.edge", "chart.topology.edgemanager" ], function(_, math, Edge, EdgeManager) {
    ...
    function createNodes(index, data) {
      ...
      var node = self.svg.group({
        ...
        if(_.typeCheck("function", self.brush.nodeImage)) {
          var nodeImages = self.brush.nodeImage.call(self.chart, data);
          for(_idx = 0, _max = nodeImages.length; _idx < _max ;  _idx++){
            /*
             * [수정]
             * IE 9, IE 10 에서 includes function을 지원하지 않음 indexof로 변경
             * */
            var imageLocation = nodeImages[_idx];
            var positionTrueX = (imageLocation.indexOf("ico_vmlist_04_in.png") > -1)?-11 :((imageLocation.indexOf("mv_computer") > -1)?-11:-(size.r-5));
            var positionTrueY = (imageLocation.indexOf("ico_vmlist_04_in.png") > -1)?-13 :((imageLocation.indexOf("mv_computer") > -1)?-13:-(size.r-5));
            var isImage =       (imageLocation.indexOf("ico_vmlist_04_in.png") > -1)?true:((imageLocation.indexOf("mv_computer") > -1)?true:false);
            self.svg.image({
              "xlink:href": nodeImages[_idx],
              /*width: isImage?30:((size.r * 2) * xy.scale),
               height: isImage?30:((size.r * 2) * xy.scale),*/
              width: isImage?22:(30 * xy.scale),
              height: isImage?22:(30 * xy.scale),
              x: positionTrueX,
              y: positionTrueY,
              cursor: "pointer"
            });
...
/*jui.define("chart.brush.polygon.scatter3d", [ "util.base", "util.math", "util.color", "chart.polygon.point" ], function(_, MathUtil, ColorUtil, PointPolygon) {
...
/*jui.define("chart.brush.polygon.column3d", [ "util.base", "util.math", "util.color", "chart.polygon.cube" ], function(_, MathUtil, ColorUtil, CubePolygon) {
...
/*jui.define("chart.brush.polygon.line3d", [ "util.base", "util.color", "util.math", "chart.polygon.point" ], function(_, ColorUtil, MathUtil, PointPolygon) {
...
jui.define("chart.brush.canvas.line.tooltip", [], function() {
  ...
  var Tooltip = function(text, x, y) {
    var self = jui.include("chart.theme." + jui.getAll()[0].options.theme);
    this.text = text || '';
    this.x = x || 0;
    this.y = y || 0;
    this.textAlign = "center";
    this.fontStyle = "600 10px Montserrat,sans-serif";
    
    this.render = function(ctx, color, opacity) {
      ctx.save();
      ctx.font = this.fontStyle;
      
      color = self.tooltipPointFontColor.replace('#', '');
...
jui.define("chart.brush.canvas.line.pathpoint", ["chart.brush.canvas.line.tooltip"], function(Tooltip) {
   ...
   * @param {Boolean} isMin
   */
  var PathPoint = function(value, x, y, isMax, isMin, target) {
    this.value = value || '';
    this.x = x || 0;
    this.y = y || 0;
    ...
    this.render = function(ctx, color, opacity) {
      ...
      var colorBackup = color.replace('#', '');
      var r = parseInt(colorBackup.substring(0,2), 16);
      var g = parseInt(colorBackup.substring(2,4), 16);
      var b = parseInt(colorBackup.substring(4,6), 16);
      ctx.fillStyle = 'rgba('+r+','+g+','+b+',1)';
      ctx.globalAlpha = opacity;
      ctx.fill();
      ctx.lineWidth=1;
      ...
      this.tooltip.render(ctx, color, opacity);
    };
  };
  
  return PathPoint;
});
jui.define("chart.brush.canvas.line", ["chart.brush.canvas.line.pathpoint"], function(PathPoint) {
  /**
  ...
  var LineBrush = function() {
    ...
    this.getPoints = function(data) {
      ...
      for (var i = 0; i < data.value.length; i++) {
        ...
        points.push(new PathPoint(this.format(value), x, y, isMax, isMin, target));
      }
      return points;
    };
    /**
    ...
    this.renderPoints = function(points, index) {
      ...
      for (var i = 0; i < points.length; i++) {
        ...
        if (prevPoint) {
          ...
          if(this.brush.activeEvents.length > 0){
            for(_idx in this.brush.activeEvents){
              if (symbol == 'curve') {
                if(this.brush.activeEvents[_idx] == currentPoint.target){
                  this.drawCurvedLine(prevPoint, currentPoint, 1);
                }else{
                  this.drawCurvedLine(prevPoint, currentPoint, 0);
                }
              }else if (symbol == 'step') {
                ...
                if(this.brush.activeEvents[_idx] == currentPoint.target){
                  this.drawLine(prevPoint, currentPoint, 1);
                }else{
                  this.drawLine(prevPoint, currentPoint, 0);
                }
              }
            }
          }else{
        ...
        if ((this.brush.display == 'all') || (this.brush.display == 'max' && currentPoint.isMax) || (this.brush.display == 'min' && currentPoint.isMin)) {
          if(this.brush.activeEvents.length > 0){
            for(_idx in this.brush.activeEvents){
              if(currentPoint.target == this.brush.activeEvents[_idx]){
                currentPoint.render(this.canvas, currentColor, 1);
                break;
              }else{
                currentPoint.render(this.canvas, currentColor, 0);
              }
            }
          }else{
            currentPoint.render(this.canvas, currentColor, 1);
    ...
    /**
     * 일반 Line 그리는 Function
     * @param prevPoint
     * @param point
     * @param opacity
     */
    this.drawLine = function(prevPoint, point, opacity) {
      this.canvas.beginPath();
    ...
    /**
     * Curved한 Line 그리는 Function
     * @param prevPoint
     * @param point
     * @param opacity
     */
    this.drawCurvedLine = function(prevPoint, point, opacity) {
      var refractionPoint = {
        x: (point.x - prevPoint.x) / 2,
        y: Math.abs((point.y - prevPoint.y) / 6)
      };
      if (point.y - prevPoint.y < 0) {
        refractionPoint.y = refractionPoint.y * -1;
      }
      this.canvas.beginPath();
      this.canvas.moveTo(prevPoint.x, prevPoint.y);
    ...
    this.draw = function() {
      var data = [];
      if (this.brush.display != undefined && this.brush.display != null || this.brush.display != ""){
        data = this.getXYCustom(true);
      }else{
        data = this.getXYCustom();
      }
  ...
  LineBrush.setup = function() {
    return {
      ...
      display: null,
      format:null,
      activeEvents:[]/*"RTT21", "MS2-1"*/
    };
  };
  
  return LineBrush;
}, "chart.brush.canvas.core");
/*jui.define("chart.brush.canvas.scatter3d", [ "util.base", "util.math", "util.color", "chart.polygon.point" ], function(_, MathUtil, ColorUtil, PointPolygon) {
  ...
  var LegendWidget = function(chart, axis, widget) {
    ...
    this.draw = function() {
      ...
      for(var i = 0; i < brushes.length; i++) {
        ...
        for(var k = 0; k < arr.length; k++) {
          ...
          if (widget.orient == "bottom" || widget.orient == "top") {
            if(widget.isOneline){
              ...
              if (x + arr[k].width > chart.area("x2") - (chart.padding("left")+chart.padding("right")+arr[k].width)) {
                x = 0;
                y += arr[k].height;
                max_height += arr[k].height;
                total_widthes.push(total_width);
        ...
        //Legend의 줄 내림이 몇번 일어나는지 확인합니다. --> Line 664
        _.setGlobalLegend(enterTime);
        if (total_width > 0) {
          total_widthes.push(total_width);
        }
      ...
      if (widget.orient == "bottom" || widget.orient == "top") {
        ...
        if (widget.align == "start") {
          ...
          if(widget.isOneline){
            ...
            x = (chart.area("x")/2) + (chart.area("width")/2 - total_width / 2);
          }
        } else if (widget.align == "end") {
...
jui.define("chart.widget.topologyctrl", [ "util.base" ], function(_) {
  ...
  var TopologyControlWidget = function() {
    ...
    function getBrushElement() {
      var children = self.chart.svg.root.get(0).children,
        index = 0,
        element = null;
