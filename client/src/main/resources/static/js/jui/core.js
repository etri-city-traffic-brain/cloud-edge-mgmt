(function (window, nodeGlobal) {
	...
	var utility = global["util.base"] = {
		...
		inherit: function (ctor, superCtor) {
			if (!this.typeCheck("function", ctor) || !this.typeCheck("function", superCtor)) {
        return false;
      }
		...
		extend: function (origin, add, skip) {
			if (!this.typeCheck([ "object", "function" ], origin)) {
        origin = {};
      }
			if (!this.typeCheck([ "object", "function" ], add)) {
        return origin;
      }
		...
		template: function (html, obj) {
			...
			if (!obj){
        return tpl(html, null, globalOpts.template);
      } else {
        return tpl(html, obj, globalOpts.template);
      }
		},
    ...
    setGlobalLegend : function(legendEnterTime){
      
      if(legendEnterTime != undefined && legendEnterTime == 0){
        globalLegend = 0;
      }else{
        globalLegend = legendEnterTime;
      }
      
    },
    
    getGlobalLegend : function(){
      return globalLegend;
    }
	};
        ...
        return false;
			...
			for(var k in attr) {
				...
				if(k.indexOf("xlink:") != -1) {
					...
					//[수정함]
					if(k == "x1" || k == "x2" || k == "y1" || k == "y2" || k == "cx" || k=="cy"){
						if(!isNaN(attr[k])){
							this.element.setAttribute(k, attr[k]);
						}else{
							this.element.setAttribute(k, 0);
						}
					}else{
            /**
             * [수정]
             * IE 9, 10에서 Line Lement에 index 또는 shape-rendering Attribute 지원하지 않음
             */
            if(navigator.userAgent.indexOf('Edge') > -1 || navigator.userAgent.indexOf('Trident') > -1){
              if(k != "shape-rendering" && k != "index"){
                this.element.setAttribute(k, attr[k]);
              }
            }else{
              this.element.setAttribute(k, attr[k]);
...
}, "util.svg.element.path"); /*util.svg.element.path.rect*/
...
}, "util.svg.element.path"); /*util.svg.element.path.symbol*/
jui.define("util.svg.element.poly", [], function() {
	...
	return PolyElement;
}, "util.svg.element.transform"); /*util.svg.element.poly*/
jui.define("util.svg.base", [ "util.base", "util.math", "util.color", "util.svg.element", "util.svg.element.transform","util.svg.element.path", "util.svg.element.path.symbol", "util.svg.element.path.rect", "util.svg.element.poly" ], function(_, math, color, Element, TransElement, PathElement, PathSymbolElement, PathRectElement, PolyElement) {
...
jui.define("util.svg.base3d", [ "util.base", "util.math", "util.color" ], function(_, math, color) {
	...
	return SVG3d;
}, "util.svg.base"); /*util.svg.base3d*/
jui.define("util.svg", [ "util.base", "util.math", "util.color", "util.svg.element", "util.svg.element.transform","util.svg.element.path", "util.svg.element.path.symbol", "util.svg.element.path.rect", "util.svg.element.poly" ], function(_, math, color, Element, TransElement, PathElement, PathSymbolElement, PathRectElement, PolyElement) {
...
jui.define("core", [ "util.base", "util.dom", "manager", "collection" ], function(_, $, UIManager, UICollection) {
...
jui.define("event", [ "jquery", "util.base", "manager", "collection" ], function($, _, UIManager, UICollection) {
