...
const system_up_time = document.getElementById("system_up_time");
...
xhr.system_up_time = new XMLHttpRequest();
...
xhr.disk_usage.send();
xhr.system_up_time.open("GET", url + "/system_up_time", false);
xhr.system_up_time.send();
...
metricData.system_up_time = JSON.parse(xhr.system_up_time.response);
metricData.system_up_time.forEach((obj) => {
    obj.unit = "Day";
    obj.chartColor = "rgba(253, 138, 138, 0.5)";
    obj.value = obj.value / 60 / 60 / 24;
});
...
function redrawChart(metric, time) {
    ...
    function refreshData(path, metricData, metricChart) {
        ...
        metricData.forEach((obj) => {
            if (metric == "cpu_usage" || metric == "system_up_time") {
                obj.chartColor = "rgba(253, 138, 138, 0.5)";
                if (metric == "system_up_time") {
                    obj.value = obj.value / 60 / 60 / 24;
                }
    ...
    switch (metric) {
        ...
        case "disk_usage":
            ...
            break;
        case "system_up_time":
            refreshData("/system_up_time?range=", metricData.system_up_time, system_up_time_chart);
            break;
        case "mem_total":
...
const system_up_time_chart = drawChart(system_up_time, metricData.system_up_time);
...
function handleTimeButtonClick(buttonId) {
    switch (buttonId) {
        ...
        case "disk-usage-30m":
            ...
            break;
        case "system-up-time-5m":
            redrawChart("system_up_time", "5m");
            break;
        case "system-up-time-10m":
            redrawChart("system_up_time", "10m");
            break;
        case "system-up-time-30m":
            redrawChart("system_up_time", "30m");
            break;
        case "mem-total-5m":
...
// [system_up_time]
document.getElementById("system-up-time-5m").addEventListener("click", () => {
    handleTimeButtonClick("system-up-time-5m");
});
document.getElementById("system-up-time-10m").addEventListener("click", () => {
    handleTimeButtonClick("system-up-time-10m");
});
document.getElementById("system-up-time-30m").addEventListener("click", () => {
    handleTimeButtonClick("system-up-time-30m");
});
